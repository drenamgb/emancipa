...
<br>
				...
				<input type="submit" Value="Voltar" style="width:140px;height:50px"/>
				</form>
				
				<form method="post" action="busca_alu_prof1.php" class="table1"><h3>
					...
					<form method="post" action="busca_alu_prof2.php" class="tablea2"><h3>
										Escola Pública: <select name="escola"   >
																<option value=""></option>
		...
		<?php	
				include("conexao.php");
				if (isset($_POST['escola']) && isset($_POST['semestre'])) {
					$escola=$_POST['escola'];
					$semestre=$_POST['semestre'];
					if (($escola != "") || ($semestre != "")){
				$sql="select*from alunos where escola like'%$escola%' and semestre like'%$semestre%'" ; 
				$execbanco=mysql_query($sql,$conexao);
						...
						echo "</table>";
							...
							echo "<p class='busca'><font color='red' size='4px'>
							Informe o tipo de Escola ou Semestre do Aluno";
